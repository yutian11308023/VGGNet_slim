...
class Vgg16():
	...
	end_time = time.time()
        print(("build model finished: %ds" % (end_time-start_time)))
        self.data_dict =None
        
    #卷积    
    def conv_layer(self, tensor, name):
        with tf.variable_scope(name):
            filt = self.get_conv_filter(name)
            conv = tf.nn.conv2d(tensor, filt, [1, 1, 1, 1], padding='SAME')
    ...
    #池化
    def max_pool(self, tensor, name):
        return tf.nn.max_pool(tensor, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name=name)
        
    def fc_layer(self, tensor, name):
        with tf.variable_scope(name):
            shape = tensor.get_shape().as_list()
            dim = 1
            for i in shape[1:]:
                dim *= i
            x = tf.reshape(tensor, [-1, dim])
                
            weights = self.get_fc_weight(name)
            biases = self.get_bias(name)
                
            result = tf.nn.bias_add(tf.matmul(x, weights), biases)
            return result
    #取w    
